...
from AYN_Package.Task.ObjectDetection.D2.YOLO.V5 import *
from AYN_Package.Optimizer.WarmUp import WarmUpOptimizer, WarmUpCosineAnnealOptimizer
from yolo_nwpu_attack.other.yolo_v5_config import YOLOV5Config
...
class YOLOV5AttackHelper:
    ...
    def restore(
        ...
        saved_dir = self.config.ABS_PATH  + '/model_pth_detector_yolov5/'
        saved_file_name = '{}yolov5_{}.pth'.format(saved_dir, epoch)
    ...
    def save(
        ...
        torch.save(self.model.state_dict(), '{}yolov5_{}.pth'.format(saved_dir, epoch))
    ...
    def show_detect_results(
        ...
        with torch.no_grad():
            saved_dir = self.config.ABS_PATH + '/eval_images_yolov5/{}/'.format(epoch)
    ...
    def show_detect_results_whole(
        ...
        with torch.no_grad():
            saved_dir = 'E:/My_ORSI_workdir/backup' + '/eval_whole_images_yolov5/{}/'.format(epoch)
    ...
    def attack_and_save(self,
        ...
        for batchid,(images,labels) in enumerate(data_loader_adversarial):
            ...
            for i in range(len(images)):
                x = BaseTool.image_tensor_to_np(images[i],self.config.data_config.mean,self.config.data_config.std)

                x_adv = BaseTool.image_tensor_to_np(images_adv[i],self.config.data_config.mean,self.config.data_config.std)
